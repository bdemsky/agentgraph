...
def testFuncA1(scheduler, i, start) -> list:
    ...
    out_var = scheduler.runPythonAgent(testFuncA2, pos=[i + 1, start], numOuts=1)
    out_var = scheduler.runPythonAgent(testFuncA3, pos=[out_var, start], numOuts=1)
    
    out_val = out_var.getValue()
...
def testFuncB1(scheduler, i, start) -> list:
    ...
    out_var = scheduler.runPythonAgent(testFuncB2, pos=[i + 1, start], numOuts=1)
    out_var2 = scheduler.runPythonAgent(testFuncB3, pos=[i + 2, start], numOuts=1)
    
    out_val = out_var.getValue()
    out_val2 = out_var2.getValue()
...
NUM_THREADS = 1
scheduler = agentgraph.getRootScheduler(model, Engine(concurrency=NUM_THREADS))
start = time.time()
out_var = None
for i in range(NUM_THREADS):
    scheduler.runPythonAgent(testFuncA1, pos=[i * 10 + 10, start])
    scheduler.runPythonAgent(testFuncB1, pos=[i * 10 + 10, start])
scheduler.shutdown()